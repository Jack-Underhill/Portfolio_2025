...
const TIMER_SCALER = 1.5; // speeds up overall motion
...
function Avatar({ avatarLogo }) {
    ...
    useEffect(() => {
        ...
        const readModalOpen = () => root.getAttribute('data-modal-open') === 'true';
        modalOpenRef.current = readModalOpen();
        ...
        const w1 = (2 * Math.PI * TIMER_SCALER) / DRIFT_X_PERIOD_MS;
        const w2 = (2 * Math.PI * TIMER_SCALER) / DRIFT_Y_PERIOD_MS;
        const w3 = (2 * Math.PI * TIMER_SCALER) / SWAY_PERIOD_MS;

        const tick = (now) => {
            const t = now - start - pausedTotalRef.current;
            ...
            floatEl.style.transform = `translate3d(${x.toFixed(2)}px, ${y.toFixed(2)}px, 0) rotate(${r.toFixed(2)}deg)`;
        ...
        const pause = () => {
            ...
            // Optional: neutralize the parallax targets so resume feels calm
            mouseTargetRef.current.x = 0;
            mouseTargetRef.current.y = 0;
            mouseCurrentRef.current.x = 0;
            mouseCurrentRef.current.y = 0;
        };
        ...
        // Watch the modal flag on <html>
        const mo = new MutationObserver(() => {
            const open = readModalOpen();
            const wasOpen = modalOpenRef.current;
            modalOpenRef.current = open;

            if (open && !wasOpen) pause();
            if (!open && wasOpen) resume();
        });
        mo.observe(root, { attributes: true, attributeFilter: ['data-modal-open'] });

        // Start loop only if modal isn't already open
        if (modalOpenRef.current) pause();
