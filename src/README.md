...
- The development-only admin renders fixed top-level sections in one route-addressable scroll stack with one primary sidebar navigation surface. Projects expands in that sidebar to expose its route-backed subsection links; no secondary sidebar or conditional content gutter remains. Each top-level section has an admin masthead, selector-backed sections use selector-only sticky toolbars, and Projects renders all fixed subsection fields for the selected record while repeated records stay selector-driven.
