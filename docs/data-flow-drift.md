...
- `ProjectAgentSourceInputs.jsx` owns the compact source file and repo controls: the GitHub repository URL input, the `+` file-picker trigger, selected source-file chips, remove actions, file-size labels, source preview trigger, per-file/repo preview status, and metadata-only preview manifest. `ProjectAgentSection.jsx` owns the pasted source-material text area, GitHub URL state, and preview request lifecycle. The browser accept list mirrors the current curated evidence contract for owner ergonomics: broad UTF-8 developer text/code/config files, safe project filenames such as `Dockerfile` and `README`, direct PDFs, direct zip bundles, and bounded public GitHub repo URLs.
