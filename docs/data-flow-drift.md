...
- `ProjectAgentSourceInputs.jsx` owns the compact source controls: the `+` file-picker trigger, selected source-file chips, remove actions, file-size labels, pasted source-material text area, source preview trigger, per-file preview status, and metadata-only preview manifest. The browser accept list mirrors the current curated evidence contract for owner ergonomics: broad UTF-8 developer text/code/config files, safe project filenames such as `Dockerfile` and `README`, direct PDFs, and direct zip bundles.
