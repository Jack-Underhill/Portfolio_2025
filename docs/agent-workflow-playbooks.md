...
- For simple current-draft work, use the Projects `Agent` subsection composer with the active draft context, an explicit owner intent, freeform owner instructions, and optional source material. The instructions field remains separate from source evidence. The source-material text area accepts pasted notes or reports, and the source `+` button opens a native multiple-file picker for curated local evidence: broad UTF-8 text/code/config files, safe project files such as `Dockerfile` or `README`, PDFs, and zip bundles containing supported text/code or PDF entries. The compact model label is read-only metadata from the local admin backend. `Revise draft` is the only editing intent: a successful revise run applies the returned patch to the unsaved local draft through `src/domain/projects/agentDraft.js` and shows a review surface with changed fields, applied fields, source manifest entries, notes, warnings, and elapsed time when available. `Review only` analyzes without editing; the server returns no patch fields to the browser and uses notes/warnings for findings, including source-backed contradictions or missing evidence when source material is present.
- Use `Preview` in the source controls before a run when selected files include PDFs, zips, or many project files. Preview calls the local admin source preview route, normalizes through the same server-owned source bundle helper as the run route, and returns a metadata-only manifest with included, skipped, truncated, and warning states. The actual run still normalizes the selected source material again instead of trusting cached preview output.
- Treat attached or pasted source material as transient evidence. It is included in the local request, normalized into a server-owned source bundle, and summarized back as a manifest without raw source text. PDF extraction uses local in-memory text extraction only; zip inspection happens in memory, skips unsafe paths, nested archives, dependency/build/generated folders, unsupported files, binary-looking entries, and over-limit entries, and never writes extracted files to disk. Source material is not uploaded to Supabase, written to disk, saved as project data, saved as run history, or exposed to the public portfolio.
...
- Tell the user to use the Projects `Agent` composer for in-admin current-draft revisions when the needed source context fits pasted source material, supported developer text/code files, PDFs, or safe zip bundles. For manual sessions or still-unsupported source types such as images, audio, video, Office documents, GitHub URLs, or deployed site URLs, paste the JSON into the admin Projects `Agent` subsection `Import draft` panel. In both flows, run `Validate Projects`, open `Preview Case Study`, and save only after review.
