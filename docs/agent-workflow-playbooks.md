...
- For simple current-draft work, use the Projects `Agent` subsection composer with the active draft context, an explicit owner intent, freeform owner instructions, and optional source material. The instructions field remains separate from source evidence. The source-material text area accepts pasted notes or reports, the source `+` button opens a native multiple-file picker for curated local evidence, and the GitHub repository URL input accepts bounded public repo evidence. Curated evidence includes broad UTF-8 developer text/code/config files, safe project files such as `Dockerfile` or `README`, PDFs, zip bundles containing supported text/code or PDF entries, and public `https://github.com/{owner}/{repo}` or `/tree/{branch-or-ref}` URLs. The compact model label is read-only metadata from the local admin backend. `Revise draft` is the only editing intent: a successful revise run applies the returned patch to the unsaved local draft through `src/domain/projects/agentDraft.js` and shows a review surface with changed fields, applied fields, source manifest entries, advisory validation preflight status when available, notes, warnings, and elapsed time when available. `Review only` analyzes without editing; the server returns no patch fields or revised-draft validation preflight to the browser and uses notes/warnings for findings, including source-backed contradictions or missing evidence when source material is present.
- Use `Preview` in the source controls before a run when selected files include PDFs, zips, many project files, or a GitHub repository URL. Preview calls the local admin source preview route, normalizes through the same server-owned source bundle helper as the run route, and returns a metadata-only manifest with included, skipped, truncated, warning, repository, ref, path, and source URL metadata where available. The actual run still normalizes the selected source material and refetches GitHub repo evidence again instead of trusting cached preview output.
