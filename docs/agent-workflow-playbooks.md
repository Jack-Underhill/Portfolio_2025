...
- For simple current-draft work, use the Projects `Agent` subsection composer with the active draft context, an explicit owner mode, freeform owner instructions, and optional source context. The default composer shows only the `Instructions` textarea, a compact `+` source context menu, a passive source summary, the `Project agent mode` select, read-only model metadata, and the send control. The `+` menu opens the supported context paths: local files and folders through the native multiple-file picker, `Add GitHub repo URL` for a bounded public repo row, and `Paste source material` for a pasted-notes row that attempts clipboard paste from the click gesture and falls back to a focused textarea. Selected files, pasted-note size, GitHub repo summary, per-file remove controls, and pre-run source preview metadata render in the full-width source context tray below the toolbar, not inside the toolbar. Curated evidence includes broad UTF-8 developer text/code/config files, safe project files such as `Dockerfile` or `README`, PDFs, zip bundles containing supported text/code or PDF entries, and public `https://github.com/{owner}/{repo}` or `/tree/{branch-or-ref}` URLs. The compact model label is read-only metadata from the local admin backend. `Revise draft` is the only editing mode: a successful revise run applies the returned patch to the unsaved local draft through `src/domain/projects/agentDraft.js` and shows a post-run review surface with changed fields, applied fields, source manifest entries, advisory validation preflight status when available, notes, warnings, and elapsed time when available. `Review only` analyzes without editing; the server returns no patch fields or revised-draft validation preflight to the browser and uses notes/warnings for findings, including source-backed contradictions or missing evidence when source context is present.
- Use `Preview context` from the source context tray before a run when selected files include PDFs, zips, many project files, or a GitHub repository URL. The action calls the local admin source preview route, normalizes through the same server-owned source bundle helper as the run route, and returns a metadata-only manifest with included, skipped, truncated, warning, repository, ref, path, and source URL metadata where available. The actual run still normalizes the selected source material and refetches GitHub repo evidence again instead of trusting cached preview output.
- Treat attached, pasted, or repo-backed source material as transient evidence. It is included in the local request, normalized into a server-owned source bundle, and summarized back as a manifest without raw source text. PDF extraction uses local in-memory text extraction only; zip inspection happens in memory, skips unsafe paths, nested archives, dependency/build/generated folders, unsupported files, binary-looking entries, and over-limit entries; GitHub ingestion happens only on the local admin backend through bounded public GitHub API metadata/tree/blob requests with no credentials, no clone, no browser-side fetch, and conservative host, timeout, count, byte, and text limits. Source material is not uploaded to Supabase, written to disk, saved as project data, saved as run history, or exposed to the public portfolio.
- Use `Retry` when the last submitted intent, instructions, pasted source, GitHub repository URL, and still-live selected source file objects should be rerun against the current active draft. Retry does not rewind to the pre-run draft; it follows the same current-draft reprompt workflow as another Run Agent submission, and it should not silently drop previously submitted source files if those browser file objects are no longer available.
...
- After either an editing `Run Agent` intent or `Import draft`, run `Validate Projects`, open `Preview Case Study`, edit if needed, and save only after review. A passed agent validation preflight is only an advisory single-draft server check; manual validation, preview, and explicit Save remain the final gates. For `Review only` runs, inspect notes/warnings first; any draft edits still need a separate revise/import/manual edit path before validation and save.
...
- Tell the user to use the Projects `Agent` composer for in-admin current-draft revisions when the needed source context fits pasted source material, supported developer text/code files, PDFs, safe zip bundles, or a bounded public GitHub repository URL. For manual sessions or still-unsupported source types such as images, audio, video, Office documents, private repos, unsupported GitHub URL shapes, or deployed site URLs, paste the JSON into the admin Projects `Agent` subsection `Import draft` panel. In both flows, run `Validate Projects`, open `Preview Case Study`, and save only after review.
