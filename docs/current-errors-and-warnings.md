...
Last full baseline checked on 2026-06-21 with Windows `cmd /c` because direct PowerShell `npm` execution is blocked by the local unsigned `npm.ps1` policy. The route-addressable, scroll-continuous admin shell with section mastheads and selector-only sticky toolbars passed lint, build, unit tests, schema drift, and the accessibility smoke.
