...
- `agent/sourceBundle.js` owns source IDs, file-count limits, aggregate source text limits, source manifests, and skipped-source warning aggregation. Source extraction remains under `agent/sourceIngestion/`: `textSource.js` owns UTF-8 text/code/notebook policy, `pdfSource.js` owns in-memory PDF text extraction through `pdf-parse`, `zipSource.js` owns in-memory archive inspection through `jszip`, and `fileSource.js` dispatches direct files by extension. These local-only dependencies do not shell out, add OCR, or expose browser-side parsing authority.
