...
- `agent/`: owns the local-only Codex bridge, controlled terminal harness, Projects agent intent/run-plan helpers, prompt/run helpers, source bundle normalization, browser-safe Codex runtime metadata, strict agent output validation, and advisory Projects validation preflight for revised drafts.
...
- `routes/projectsAgent.js`: exposes the local-only `POST /admin-api/projects/agent/run`, `POST /admin-api/projects/agent/sources/preview`, and `GET /admin-api/projects/agent/runtime` routes, accepts JSON or multipart source-backed requests including optional GitHub repo URLs, and delegates source bundling, Codex orchestration, and runtime metadata resolution to `agent/`.
...
- Successful `revise` runs include an advisory `validationPreflight` result when the normalized patch can be applied to a temporary active draft and checked with existing Projects validation rules. Preflight failures remain successful run-result data for owner review; they do not trigger a second Codex call, block patch auto-application, save data, or replace manual `Validate Projects`, `Preview Case Study`, and explicit Save.
...
- Source material for Projects agent runs is transient request context. Browser no-file, pasted-source-only, and GitHub-repo-only requests stay JSON; requests with selected local source files use multipart with a JSON `payload` part and repeated `sourceFiles` entries. `POST /admin-api/projects/agent/sources/preview` uses the same request shape and source bundle helper as the run route, returns only manifest/count/warning/limit metadata, and never invokes Codex.
- `agent/sourceBundle.js` owns source IDs, file-count limits, aggregate source text limits, source manifests, GitHub repo URL delegation, and compact skipped-source warning aggregation. Source extraction remains under `agent/sourceIngestion/`: `fileSource.js` dispatches direct files, `textSourcePolicy.js` owns text filename/media/disallowed-name policy, `textSource.js` owns UTF-8 text/code/notebook normalization, `pdfSource.js` owns in-memory PDF text extraction through `pdf-parse`, `zipSource.js` and `zipEntryNormalizer.js` own in-memory archive inspection through `jszip`, `githubSource.js` and focused `github/` modules own public GitHub URL parsing, API fetch handling, path filtering, high-signal tree selection, manifest metadata, limits, and file normalization, `validation/` owns reusable file/text/byte validation helpers, and `sourcePathUtils.js` plus `sourceResult.js` own shared path/result primitives. These local-only dependencies do not shell out, add OCR, clone repositories, write fetched files to disk, or expose browser-side parsing/fetching authority.
- GitHub source ingestion supports bounded public `github.com/{owner}/{repo}` repository URLs and `/tree/{branch-or-ref}` URLs. The backend fetches repository metadata, tree metadata, and selected blob contents through the public GitHub API with no credentials or token storage, rejects unsupported URL shapes and hosts, applies a 500 tree-entry inspection cap, 40 included-file cap, 512 KB per-file cap, 5 MB total fetched-byte cap, 80,000 character GitHub text cap, 10 second fetch timeout, and the shared total source text cap. Dependency/build/generated/noisy paths are ignored before content fetch when possible, and unauthenticated rate-limit or network failures are reported as metadata-only warnings.
- Supported curated evidence includes pasted text, broad UTF-8 developer text/code/config files, safe extensionless project files such as `Dockerfile` and `README`, direct PDFs, direct zip archives containing supported text/code or PDF entries, and bounded public GitHub repositories. The server rejects or skips secret-like names, binary/database dumps, generated/minified bundles, unsupported extensions, unreadable/undecodable files, nested archives, unsafe zip paths, dependency/build/generated zip folders, ignored GitHub paths, unsupported repo URL shapes, over-limit files, and failed GitHub fetches with metadata-only warnings. Extracted source text, zip entries, and fetched GitHub file contents are not written to disk, uploaded to Supabase, persisted as project data, saved as run history, or returned to the browser in preview or run results.
